...
const selectedAttribute = new URLSearchParams(window.location.search).get('attribute') || '';
const selectedForm = new URLSearchParams(window.location.search).get('form') || '';
const selectedSingular = new URLSearchParams(window.location.search).get('singular') || '';
const selectedPlural = new URLSearchParams(window.location.search).get('plural') || '';

const hasAttribute = () => {
    const allowedCategories = ['', 'verb', 'noun', 'adjective', 'determiner'];
    return allowedCategories.includes(selectedCategory.value);
};
...
const hasForm = () => {
    ...
    return allowedCategories.includes(selectedCategory.value) && allowedSingPatterns.includes(selectedSingular.value) && !disallowedAttributes.includes(selectedAttribute.value);
};
...
const hasPlural = () => {
    ...
    return allowedCategories.includes(selectedCategory.value) && !disallowedAttributes.includes(selectedAttribute.value);
};

const isRegular = () => {
    const allowedSingPatterns = ['CiCiC', 'CCīC', 'relative', 'ia'];
    return allowedSingPatterns.includes(selectedSingular.value);
};
...
onMounted(() => {
    // turning this off so that I can manually order & annotate the options
    // SearchStore.fetchFilterOptions();
});
</script>

<template>
    <div class="sg-filters">
        <select v-model="SearchStore.filters.category" :class="SearchStore.filters.category ? 'persisting' : ''"
                @change="SearchStore.updateFilter('category', $event.target.value)">
        ...
        <select v-model="SearchStore.filters.attribute" :class="SearchStore.filters.attribute ? 'persisting' : ''"
                @change="SearchStore.updateFilter('attribute', $event.target.value)">
        ...
        <select v-model="SearchStore.filters.form" :class="SearchStore.filters.form ? 'persisting' : ''"
                @change="SearchStore.updateFilter('form', $event.target.value)">
        ...
        <select v-model="SearchStore.filters.singular" :class="SearchStore.filters.singular ? 'persisting' : ''"
                @change="SearchStore.updateFilter('singular', $event.target.value)">
        ...
        <select v-model="SearchStore.filters.plural" :class="SearchStore.filters.plural ? 'persisting' : ''"
                @change="SearchStore.updateFilter('plural', $event.target.value)">
